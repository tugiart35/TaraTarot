...
async function applySEOOptimization() {
  ...
  try {
    // SEO verilerini yükle - daha güvenli parse
    console.log('📖 SEO verileri yükleniyor...');
    let seoDataRaw = fs.readFileSync(SEO_DATA_FILE, 'utf-8');

    // JSON parse hatası için temizleme
    seoDataRaw = seoDataRaw.trim();
    if (!seoDataRaw.endsWith(']')) {
      console.log('⚠️ JSON formatı düzeltiliyor...');
      seoDataRaw = seoDataRaw + ']';
    }
    ...
    for (const seoCard of seoData) {
      try {
        ...
        if (!cardId) {
          console.log(`⚠️ ${JSON.stringify(seoCard.card)}: Kart ID bulunamadı`);
          continue;
        }
        ...
        console.log(`❌ ${JSON.stringify(seoCard.card)}: ${error.message}`);
      }
    }
    ...
    console.log(`\n📊 Özet:`);
    console.log(`✅ ${cardsProcessed} kart işlendi`);
