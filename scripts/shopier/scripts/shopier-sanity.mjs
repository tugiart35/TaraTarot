...
const {
  SHOPIER_LOCAL_URL = 'http://localhost:3000/api/shopier/callback',
  SHOPIER_SECRET,
  SUPABASE_URL,
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  console.error(
    'SUPABASE_URL and SUPABASE_SERVICE_KEY are required as env vars.'
  );
  process.exit(1);
}
...
async function postWebhook(payload) {
  ...
  if (SHOPIER_SECRET)
    headers['x-shopier-signature'] = sign(body, SHOPIER_SECRET);
...
(async () => {
  ...
  try {
    ...
    console.log(
      `Polling Supabase for table=${EXPECTED_TABLE} column=${EXPECTED_COLUMN} value=${TEST_MERCHANT_ORDER_ID} ...`
    );
    ...
    if (poll.row.user_id || USER_ID) {
      ...
      if (uerr) console.warn('User lookup error:', uerr.message || uerr);
        ...
        console.log(
          'User credits:',
          udata && udata[0] ? udata[0] : 'not found'
        );
    }

    console.log(
      'Email send check: (if you persist mail logs) querying email_logs ...'
    );
    try {
      const { data: mailData } = await supabase
        .from('email_logs')
        .select('*')
        .eq('merchant_order_id', TEST_MERCHANT_ORDER_ID)
        .limit(1);
      if (mailData && mailData.length)
        console.log('Email log found:', mailData[0]);
      else console.log('No email log found (may be normal).');
    } catch (err) {
      console.warn(
        'Email-log check skipped (table may not exist):',
        err.message || err
      );
    }
