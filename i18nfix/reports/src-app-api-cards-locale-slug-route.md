...
### Purpose
Bu dosya, tarot kartı verilerini JSON API endpoint'i üzerinden sunan RESTful API handler'dır. Locale ve slug parametrelerine göre kart datasını döndürür.
...
2. ⚠️ **Hardcoded Error Messages**: English-only (could be i18n for better client integration)
...
**Recommendation:** Keep English for now (API standard practice). Localization is optional.
...
### i18n for API Routes
**Note:** API error messages are typically in English (industry standard). i18n for API responses is optional and usually not recommended unless:
