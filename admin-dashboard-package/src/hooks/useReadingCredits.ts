...
import {
  ...
  ReadingType,
} from '../constants/reading-credits';
...
export const useReadingCredits = (readingType: ReadingType) => {
  ...
  const [creditStatus, setCreditStatus] = useState<CreditStatus>({
    ...
    requiredCredits: READING_CREDIT_CONFIGS[readingType]?.cost || 0,
    currentCredits: 0,
  });
  ...
  useEffect(() => {
    ...
    const checkCredits = async () => {
      ...
      try {
        ...
        const requiredCredits = READING_CREDIT_CONFIGS[readingType]?.cost || 0;
