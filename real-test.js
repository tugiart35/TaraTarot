...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF0bG9rZGtjZXJqcmJydHBobHJoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgxNDgwOTcsImV4cCI6MjA3MzcyNDA5N30.ezpkbTze481HDVgWDohuDY-iOfK8TQpog1Jzk6Glm6k';
...
const generateRandomQuestions = (spreadType) => {
  ...
  return {
    concern: questions.concerns[Math.floor(Math.random() * questions.concerns.length)],
    understanding: questions.understandings[Math.floor(Math.random() * questions.understandings.length)],
    emotional: questions.emotionals[Math.floor(Math.random() * questions.emotionals.length)]
  };
};
...
async function createRealReading(readingType, communicationMethod) {
  ...
  const readingTypeKey = readingType === 'VOICE' ? 'PROBLEM_SOLVING_DETAILED' : 'PROBLEM_SOLVING_WRITTEN';
  ...
  try {
    ...
    console.log(`💰 Kredi Maliyeti: ${readingData.cost_credits} (${readingTypeKey})`);
...
async function sendTestResults(voiceReading, writtenReading, initialCredits, finalCredits) {
  ...
  const results = {
    ...
    credits: {
      ...
      totalCredits: CREDIT_COSTS.PROBLEM_SOLVING_DETAILED + CREDIT_COSTS.PROBLEM_SOLVING_WRITTEN
    },
    summary: {
      totalReadings: 2,
      success: true,
  ...
  console.log(`🎤 Sesli Okuma: ${results.voiceReading.id} (${results.voiceReading.communicationType} - ${results.voiceReading.communicationMethod})`);
  console.log(`📝 Yazılı Okuma: ${results.writtenReading.id} (${results.writtenReading.communicationType} - ${results.writtenReading.communicationMethod})`);
  console.log(`💳 Kredi: ${results.credits.initial} → ${results.credits.final} (-${results.credits.deducted})`);
  console.log(`🎤 Sesli Okuma Maliyeti: ${results.credits.voiceCredits} kredi`);
  console.log(`📝 Yazılı Okuma Maliyeti: ${results.credits.writtenCredits} kredi`);
  console.log(`💰 Toplam Maliyet: ${results.credits.totalCredits} kredi`);
  console.log(`📊 Toplam Kart: ${results.voiceReading.cards + results.writtenReading.cards}`);
...
async function runRealTest() {
  ...
  try {
    ...
    // 5. Sonuçları ilet
    await sendTestResults(voiceReading, writtenReading, initialCredits, finalCredits);
