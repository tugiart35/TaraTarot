...
serve(async (req) => {
  ...
  if (expected !== signature) return badRequest('Invalid signature', 401)
  ...
  const eventId = String(payload.transactionId || payload.id || payload.event_id || '')
  ...
  if (existing) return ok({ idempotent: true })

  // Insert event
  const { error: evErr } = await supabase
    .from('webhook_events')
    .insert({ provider: PROVIDER, event_id: eventId, payload })
  if (evErr) return badRequest('Event insert failed: ' + evErr.message)
  ...
  try {
    ...
    if (txErr) return badRequest('user_transactions insert failed: ' + txErr.message)
    ...
    if (awardErr) return badRequest('Credit award failed: ' + awardErr.message)
