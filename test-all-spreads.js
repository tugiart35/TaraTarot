...
async function testSingleSpread(spreadType) {
  ...
  try {
    ...
    // Sesli okuma
    const voiceReading = await createReading(spreadType, 'VOICE', questions, cards);
    
    // Yazılı okuma
    const writtenReading = await createReading(spreadType, 'WRITTEN', questions, cards);
    ...
    console.error(`❌ ${SPREAD_CONFIGS[spreadType].spreadName} test başarısız:`, error.message);
    return {
      spreadType,
...
async function testAllSpreads() {
  ...
  console.log('📊 Test edilecek açılımlar:', Object.keys(SPREAD_CONFIGS).join(', '));
  ...
  if (totalCreditsUsed > 0) {
    ...
    console.log(`💳 Kredi: ${initialCredits} → ${finalCredits} (-${totalCreditsUsed})`);
    ...
    console.log(`🎯 Genel Başarı: ${testResults.summary.success ? '✅ Evet' : '❌ Hayır'}`);
    
    console.log('\n📊 Açılım Detayları:');
    results.forEach(result => {
      const status = result.success ? '✅' : '❌';
      console.log(`${status} ${result.spreadName}: ${result.success ? 'Başarılı' : result.error}`);
    });
