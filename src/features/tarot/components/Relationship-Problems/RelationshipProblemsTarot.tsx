...
---
Dosya Amacı:
- İlişki Sorunları tarot açılımı ana bileşeni
- 9 kartlık özel Venn diyagramı düzeni ile ilişki sorunları analizi
- Kullanıcı kart seçer, pozisyonları yönetir, yorum alır

Bağlı Dosyalar:
- relationship-problems-config.ts (konfigürasyon)
...
import {
  ...
  RELATIONSHIP_PROBLEMS_CARD_COUNT,
} from './relationship-problems-config';
...
export default function RelationshipProblemsReading({
  ...
  const { user } = useAuth();
  const relationshipProblemsSpread = findSpreadById(
    'relationship-problems-spread'
  );

  // Kredi yönetimi
  const detailedCredits = useReadingCredits('RELATIONSHIP_PROBLEMS_DETAILED');
  const writtenCredits = useReadingCredits('RELATIONSHIP_PROBLEMS_WRITTEN');
  ...
  const {
    ...
    onComplete: (_cards, _interpretation) => {
      // İlişki sorunları açılımı tamamlandı
    },
  ...
  if (!relationshipProblemsSpread) {
    return (
      <div className='text-red-500'>
        İlişki Sorunları Açılımı konfigürasyonu bulunamadı. Lütfen
        tarotSpreads.ts dosyasını kontrol edin.
      </div>
    );
  }

  // Pozisyona özel kart anlamını al
  const getRelationshipProblemsCardMeaning = (
    card: TarotCard | null,
    position: number,
    isReversed: boolean
  ): string => {
  ...
  const generateBasicInterpretation = (): string => {
    ...
    if (
      selectedCards.length !== RELATIONSHIP_PROBLEMS_CARD_COUNT ||
      selectedCards.some(c => !c)
    ) {
      return 'Tüm kartları seçmeden yorum oluşturulamaz.';
    }
    let interpretation = `💔 **İlişki Sorunları Açılımı**\n\n`;
    if (userQuestion.trim()) {
      interpretation += `**Sevgili danışan,** ilişki sorunları analizi "${userQuestion}" için özel hazırlanmış analiz:\n\n`;
    }
    RELATIONSHIP_PROBLEMS_POSITIONS_INFO.forEach((posInfo, index) => {
      const card = cards[index];
      const reversed = !!isReversed[index];
      if (card) {
        interpretation += `**${posInfo.id}. ${posInfo.title}: ${card.nameTr}** (${reversed ? 'Ters' : 'Düz'})\n*${posInfo.desc}*\n${getRelationshipProblemsCardMeaning(card, posInfo.id, reversed)}\n\n`;
      }
    });
    interpretation += `💫 **${t('tarotPage.relationshipProblemsSpread.summary')}:**\n"${t('tarotPage.relationshipProblemsSpread.summaryText')}"`;
  ...
  return (
    <div className='w-full space-y-6 md:space-y-8'>
      ...
      <div className='w-full relative overflow-hidden rounded-2xl shadow-2xl bg-gradient-to-br from-blue-900/90 via-slate-900/80 to-cyan-800/80 border border-blue-700/60'>
        ...
        <div className='relative z-10 p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8'>
          <div className='relative w-full h-full min-h-[400px] xs:min-h-[450px] sm:min-h-[500px] md:min-h-[550px] lg:min-h-[600px] xl:min-h-[650px]'>
            {RELATIONSHIP_PROBLEMS_POSITIONS_LAYOUT.map((position, idx) => (
              <BaseCardPosition
                key={position.id}
                position={{
                  id: idx + 1,
                ...
                positionInfo={
                  RELATIONSHIP_PROBLEMS_POSITIONS_INFO[idx] ?? {
      ...
      {selectedReadingType &&
        currentPosition &&
        currentPosition <= RELATIONSHIP_PROBLEMS_CARD_COUNT && (
              ...
              <div className='flex items-center space-x-3'>
                ...
                <div className='text-center'>
                  <div className='text-blue-200 font-bold text-lg'>
                    {RELATIONSHIP_PROBLEMS_POSITIONS_INFO[currentPosition - 1]
                      ?.title ?? ''}
                  </div>
      ...
      {selectedCards.filter(c => c !== null).length > 0 && (
        <div className='flex justify-center'>
          ...
          >
            {t('relationshipProblems.form.clearAll')}
          </button>
        </div>
      )}
          ...
          <div ref={interpretationRef} className='space-y-6'>
            ...
            {(selectedReadingType === READING_TYPES.DETAILED ||
              ...
              <div className='flex justify-center mt-8'>
                ...
                >
                  {isSavingReading
                    ? t('relationshipProblems.modals.savingReading')
                    : t('relationshipProblems.modals.saveReading')}
                </button>
              </div>
            )}
          </div>
        ...
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          <div className='bg-gradient-to-br from-blue-900/95 to-cyan-900/95 border border-blue-500/30 rounded-3xl shadow-2xl max-w-md w-full p-8 text-center'>
            ...
            <h2 className='text-2xl font-bold text-green-400 mb-4'>
              {t('relationshipProblems.modals.successTitle')}
            </h2>

            {/* Mesaj */}
            <p className='text-blue-200 mb-6 leading-relaxed'>
              {t('relationshipProblems.modals.successMessage')}
            </p>

            {/* Bilgi */}
            <div className='bg-blue-800/30 border border-blue-500/20 rounded-xl p-4 mb-6'>
              <p className='text-blue-300 text-sm'>
                {t('relationshipProblems.modals.redirecting')}
              </p>
            </div>
      ...
      {showCardDetails && (
        <CardDetails
          ...
          positionInfo={(() => {
            ...
            const p = RELATIONSHIP_PROBLEMS_POSITIONS_INFO[idx];
            return p
