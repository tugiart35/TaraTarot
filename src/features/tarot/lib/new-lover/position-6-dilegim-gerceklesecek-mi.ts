...
export const position6Meanings: NewLoverposition6Meaning[] = [
  ...
  // --- Tılsımlar Serisi ---//
    ...
    },
    {
      ...
      reversed:
        'Ters prens, erteleme ve dağınık odak dileğinizi engelleyebilir.',
    ...
    },
    {
      ...
      reversed:
        'Ters kraliçe, öz bakımı ihmal etmek dileğinizi zorlaştırabilir.',
...
export const getI18nposition6Meaning = (
  ...
  const cardKey = cardName
    ...
    .replace(/[^a-z0-9]/g, '');
  const i18nUpright = t(
    `new-lover.meanings.${cardKey}.position6.upright`
  );
  const i18nReversed = t(
    `new-lover.meanings.${cardKey}.position6.reversed`
  );
  const i18nKeywords = t(
    `new-lover.meanings.${cardKey}.position6.keywords`
  );
  const i18nContext = t(
    `new-lover.meanings.${cardKey}.position6.context`
  );
  const i18nGroup = t(
