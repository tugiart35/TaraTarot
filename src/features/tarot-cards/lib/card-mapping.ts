...
export class CardMapping {
  ...
  static getCardSlugForLocale(
    ...
    if (majorArcanaMapping[cardKey as keyof typeof majorArcanaMapping]) {
      return majorArcanaMapping[cardKey as keyof typeof majorArcanaMapping][locale];
    }
  ...
  private static getMinorArcanaSlug(cardKey: string, locale: 'tr' | 'en' | 'sr'): string {
    const suit = cardKey.includes('cups') ? 'cups' :
                 cardKey.includes('pentacles') ? 'pentacles' :
                 cardKey.includes('swords') ? 'swords' :
                 cardKey.includes('wands') ? 'wands' : '';
    ...
    const number = cardKey.includes('ace') ? 'ace' :
                   ...
                   cardKey.includes('queen') ? 'queen' :
                   cardKey.includes('king') ? 'king' : '';

    if (locale === 'tr') {
      const suitNames: Record<string, string> = { cups: 'kupalar', pentacles: 'yildizlar', swords: 'kiliclar', wands: 'asalar' };
      ...
      const suitNames: Record<string, string> = { cups: 'kupa', pentacles: 'novcic', swords: 'mace', wands: 'stap' };
      const numberNames: Record<string, string> = {
  ...
  private static getMinorArcanaKey(slug: string, locale: 'tr' | 'en' | 'sr'): string | null {
    if (locale === 'tr') {
