...
// Reading type - dashboard.types'dan import edilmeli
...
const getCardMeanings = (reading: Reading) => {
  if (
    !reading.cards ||
    !['love', 'LOVE_SPREAD_WRITTEN', 'LOVE_SPREAD_DETAILED'].includes(
  ...
  try {
    // reading.cards zaten string olarak geliyor (JSONB'den)
    ...
    if (Array.isArray(cardsArray)) {
      return cardsArray.map((card: any, index: number) => {
        ...
        const cardName =
          card.nameTr || card.name || card.card || `Kart ${position}`;
        ...
        {recentReadings.length > 0 ? (
          <div className='space-y-4'>
            {recentReadings.map(reading => {
              ...
              // Başlık Reading interface'inden alınıyor
              const title = reading.title || 'Mistik Okuma';
                ...
                <div key={reading.id} className='bg-crystal-clear rounded-lg'>
                  <div className='flex items-center space-x-4 p-4'>
                    <div className={`p-2 rounded-lg ${typeInfo.bgColor}`}>
                      <IconComponent
                        className={`h-5 w-5 ${typeInfo.iconColor}`}
                      />
                    </div>
                    ...
                    <div className='px-4 pb-4 border-t border-cosmic-fog/50'>
                      <div className='pt-4 space-y-3'>
                        ...
                        {cardMeanings.map((card, index) => (
                          <div
                            key={index}
                            className='bg-cosmic-black/50 rounded-lg p-3'
                          >
          ...
          <div className='text-center py-8'>
            ...
            <a href={`/${currentLocale}/tarot`} className='btn btn-primary'>
              Okuma Başlat
            </a>
          </div>
        )}
