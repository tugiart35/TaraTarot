...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      let query = supabase
        .from('profiles')
        .select('id, email, display_name, credit_balance, created_at, is_admin, is_premium', { count: 'exact' })
      ...
      const formattedUsers = (data || []).map((user: any) => ({
        ...
        status: user.is_admin ? 'admin' : (user.is_premium ? 'premium' : 'active')
      }));
          ...
          <div key={user.id} className="admin-card rounded-2xl p-6 admin-hover-lift">
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  {user.status === 'admin' ? 'Admin' : user.status === 'premium' ? 'Premium' : 'Aktif'}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-700">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  onClick={() => user.status === 'admin' ? null : (user.status === 'premium' ? showBanConfirmation(user) : showBanConfirmation(user))}
                  ...
                  title={user.status === 'admin' ? 'Admin kullanıcı' : user.status === 'premium' ? 'Premium kullanıcı' : 'Kullanıcıyı Askıya Al'}
