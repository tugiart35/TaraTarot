...
export default function SettingsPage() {
  ...
  // Shopier state
  const [shopierConfig, setShopierConfig] = useState<ShopierConfig>({
    merchantId: '',
    apiKey: '',
    apiSecret: '',
    testMode: true,
  ...
  const [shopierTestResult, setShopierTestResult] = useState<string>('');
  ...
  const testShopierConnection = async () => {
    setShopierTestResult('Test ediliyor...');
    try {
      // Test webhook endpoint'ini çağır
      const response = await fetch('/api/webhook/shopier', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          platform_order_id: 'TEST_CONNECTION',
          status: 'success',
      ...
      if (response.ok) {
        setShopierTestResult('✅ Shopier bağlantısı başarılı!');
      } else {
        setShopierTestResult('❌ Shopier bağlantısı başarısız!');
      }
    } catch (error) {
      setShopierTestResult('❌ Test hatası: ' + error);
    }
  };

  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text).then(() => {
      setSavedMessage('URL kopyalandı!');
        ...
        {activeTab === 'shopier' && (
          <div>
            ...
            <div className="admin-card rounded-2xl p-6 mb-6">
              <h4 className="text-lg font-semibold text-white mb-4">Temel Ayarlar</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <input
                    type="text"
                    value={shopierConfig.merchantId}
                    onChange={(e) => setShopierConfig({...shopierConfig, merchantId: e.target.value})}
                    className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="Shopier Merchant ID"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="password"
                    value={shopierConfig.apiKey}
                    onChange={(e) => setShopierConfig({...shopierConfig, apiKey: e.target.value})}
                    className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div>
                  ...
                  <input
                    type="password"
                    value={shopierConfig.apiSecret}
                    onChange={(e) => setShopierConfig({...shopierConfig, apiSecret: e.target.value})}
                    className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div className="flex items-center space-x-3">
                  <input
                    type="checkbox"
                    id="testMode"
                    checked={shopierConfig.testMode}
                    onChange={(e) => setShopierConfig({...shopierConfig, testMode: e.target.checked})}
                    className="w-4 h-4 text-purple-600 rounded focus:ring-purple-500"
                  />
                  <label htmlFor="testMode" className="text-sm text-slate-300">
                    Test Modu
                  </label>
                </div>
              </div>
            ...
            {/* URL Ayarları */}
            <div className="admin-card rounded-2xl p-6 mb-6">
              <h4 className="text-lg font-semibold text-white mb-4">URL Ayarları</h4>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="url"
                    value={shopierConfig.webhookUrl}
                    onChange={(e) => setShopierConfig({...shopierConfig, webhookUrl: e.target.value})}
                    className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="https://yourdomain.com/api/webhook/shopier"
                  />
                  <p className="text-xs text-slate-400 mt-1">
                    Shopier dashboard'ında bu URL'yi webhook olarak ayarlayın
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="url"
                      value={shopierConfig.successUrl}
                      onChange={(e) => setShopierConfig({...shopierConfig, successUrl: e.target.value})}
                      className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                  ...
                  <div>
                    ...
                    <input
                      type="url"
                      value={shopierConfig.cancelUrl}
                      onChange={(e) => setShopierConfig({...shopierConfig, cancelUrl: e.target.value})}
                      className="w-full p-3 admin-glass rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
            ...
            {/* Paket Linkleri */}
            <div className="admin-card rounded-2xl p-6 mb-6">
              <h4 className="text-lg font-semibold text-white mb-4">Paket Linkleri</h4>
              
              <div className="space-y-3">
                <div className="admin-glass rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h5 className="font-medium text-white">100 Kredi Paketi</h5>
                      <p className="text-sm text-slate-400">Basic Paket</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-300">https://www.shopier.com/38014526</p>
                      <button 
                        onClick={() => copyToClipboard('https://www.shopier.com/38014526')}
                ...
                <div className="admin-glass rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h5 className="font-medium text-white">300 Kredi Paketi</h5>
                      <p className="text-sm text-slate-400">Premium Paket</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-300">https://www.shopier.com/38014551</p>
                      <button 
                        onClick={() => copyToClipboard('https://www.shopier.com/38014551')}
                        className="text-purple-400 hover:text-purple-300 text-sm mt-1"
                      >
                        Kopyala
                      </button>
                ...
                <div className="admin-glass rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h5 className="font-medium text-white">500 Kredi Paketi</h5>
                      <p className="text-sm text-slate-400">Popüler Paket</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-300">https://www.shopier.com/38014558</p>
                      <button 
                        onClick={() => copyToClipboard('https://www.shopier.com/38014558')}
                        className="text-purple-400 hover:text-purple-300 text-sm mt-1"
                      >
                        Kopyala
                      </button>
            ...
            <div className="admin-card rounded-2xl p-6">
              <h4 className="text-lg font-semibold text-white mb-4">Test ve Kaydet</h4>
              
              <div className="flex flex-col sm:flex-row gap-4 mb-4">
                <button
                  onClick={testShopierConnection}
              ...
              {shopierTestResult && (
                <div className="admin-glass rounded-lg p-4">
                  <p className="text-sm text-slate-300">{shopierTestResult}</p>
                </div>
              )}

              <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <h5 className="font-medium text-blue-400 mb-2">Kurulum Adımları:</h5>
                <ol className="text-sm text-slate-300 space-y-1 list-decimal list-inside">
                  <li>Shopier dashboard'ında her paket için Success/Cancel URL'lerini ayarlayın</li>
                  <li>Webhook URL'ini Shopier'e ekleyin</li>
                  <li>Merchant ID, API Key ve Secret'ı girin</li>
                  <li>Test modunu aktif edin ve test edin</li>
                  <li>Test başarılıysa production moduna geçin</li>
                </ol>
              </div>
            </div>
          </div>
