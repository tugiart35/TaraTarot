...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const totalCredits = (allProfiles || []).reduce((sum, profile) => sum + (profile.credit_balance || 0), 0);
      ...
      const dailyCreditUsage = (dailyTransactions || []).reduce((sum, transaction) => 
        sum + Math.abs(transaction.delta_credits), 0);

      // Günlük gelir hesaplama (Shopier'den)
      const { data: dailyRevenueData, error: dailyRevenueError } = await supabase
        .from('transactions')
        .select('amount')
        .eq('type', 'purchase')
      ...
      const dailyRevenue = (dailyRevenueData || []).reduce((sum, transaction) => 
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-base">
              Sistem durumunu kontrol edin, istatistikleri inceleyin ve platformu yönetin
            </p>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="admin-glass rounded-xl p-5 admin-hover-scale">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <span className="text-sm font-medium text-slate-300">Son Giriş</span>
            </div>
            <div className="text-white font-semibold text-lg">{new Date().toLocaleString('tr-TR')}</div>
            <div className="text-xs text-slate-500 mt-1">Admin paneli erişimi</div>
          </div>
          <div className="admin-glass rounded-xl p-5 admin-hover-scale">
            <div className="flex items-center space-x-3 mb-3">
              <Zap className="h-5 w-5 text-green-400" />
              <span className="text-sm font-medium text-slate-300">Sistem Durumu</span>
            </div>
            <div className="text-green-400 font-semibold text-lg flex items-center">
              <span className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse"></span>
              Tüm Sistemler Aktif
            </div>
            <div className="text-xs text-slate-500 mt-1">Veritabanı ve API bağlantıları</div>
          </div>
          <div className="admin-glass rounded-xl p-5 admin-hover-scale">
            <div className="flex items-center space-x-3 mb-3">
              <Target className="h-5 w-5 text-purple-400" />
              <span className="text-sm font-medium text-slate-300">Günlük Kredi Harcama</span>
            </div>
            <div className="text-purple-400 font-semibold text-lg">{stats.dailyCreditUsage} Kredi</div>
            <div className="text-xs text-slate-500 mt-1">Bugün harcanan toplam kredi</div>
          </div>
        </div>
      </div>
