...
export default function AdminLayout({
  ...
  const checkAdminStatus = async () => {
    try {
      // Auth sayfasındaysa admin kontrolü yapma - döngüyü önle
      if (pathname === `/${locale}/pakize/auth`) {
        setLoading(false);
        return;
      }
      ...
      if (!session?.user) {
        // Giriş yapmamış kullanıcıyı auth sayfasına yönlendir
        if (!isRedirecting) {
      ...
      // Supabase'den admin durumunu kontrol et
      ...
      if (error || !profile?.is_admin) {
        console.log(
          '❌ Admin yetkisi yok:',
          error?.message || 'is_admin false'
        );
        // Admin olmayan kullanıcıyı auth sayfasına yönlendir
        if (!isRedirecting) {
          setIsRedirecting(true);
          router.push(`/${locale}/pakize/auth`);
        }
      ...
      console.log('✅ Admin yetkisi onaylandı:', profile.email);
    } catch (error) {
  ...
  // Auth sayfasındaysa loading gösterme, direkt children'ı render et
  if (pathname === `/${locale}/pakize/auth`) {
    return <>{children}</>;
  }
  ...
  // Admin kontrolü artık otomatik yönlendirme ile yapılıyor
  if (!isAdmin) {
    return null; // Yönlendirme yapılırken loading göster
  }
