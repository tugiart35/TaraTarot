...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (searchTerm) {
        countQuery = countQuery.or(`ref_id.ilike.%${searchTerm}%,id.ilike.%${searchTerm}%,profiles.email.ilike.%${searchTerm}%,profiles.display_name.ilike.%${searchTerm}%`);
      }
      ...
      // Arama terimi - kullanıcı email ve display_name dahil
      if (searchTerm) {
        query = query.or(`ref_id.ilike.%${searchTerm}%,id.ilike.%${searchTerm}%,profiles.email.ilike.%${searchTerm}%,profiles.display_name.ilike.%${searchTerm}%`);
      }
      ...
      const formattedOrders = (data || []).map(transaction => ({
        ...
        user_display_name: transaction.profiles?.display_name || 
  ...
  const handleExportOrders = async () => {
    try {
      ...
      const csvData = (data || []).map(transaction => ({
        ...
        'Kullanıcı': transaction.profiles?.display_name || transaction.profiles?.full_name || 'Bilinmeyen',
