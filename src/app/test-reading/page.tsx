...
export default function TestReadingPage() {
  ...
  const testRPC = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.rpc('fn_create_reading_with_debit', {
        p_user_id: '9c67169b-a114-4d40-95bd-16c30e1f98b4',
        p_reading_type: 'love',
        p_spread_name: 'Test Yayılımı',
        p_title: 'Test Okuma',
        p_interpretation: 'Bu bir test okumasıdır.',
        p_cards: [{ id: 1, name: 'The Fool', nameTr: 'Budala', isReversed: false }],
        p_questions: {
          personalInfo: { name: 'Test', surname: 'Kullanıcı', birthDate: '1990-01-01', email: 'test@example.com' },
          userQuestions: { concern: { question: 'Test', answer: 'Test cevap' } }
        },
        p_cost_credits: 0,
      ...
      setResult({ error: err instanceof Error ? err.message : 'Bilinmeyen hata' });
    } finally {
      setLoading(false);
    }
  };
