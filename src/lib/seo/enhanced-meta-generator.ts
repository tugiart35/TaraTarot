...
export function generateEnhancedMetadata(
  ...
  const { title, description, keywords, canonical, ogImage, ogType = 'website', noindex, nofollow } = config;
...
export function generateCardPageMetadata(
  ...
  const localePrefix = locale === 'tr' ? 'kartlar' : locale === 'en' ? 'cards' : 'kartice';
...
export function generateReadingPageMetadata(
  ...
  const name = readingNames[readingType as keyof typeof readingNames]?.[locale as 'tr' | 'en' | 'sr'] || 'Tarot Reading';
