...
const generateRandomQuestions = spreadType => {
  ...
  return {
    concern:
      questions.concern[Math.floor(Math.random() * questions.concern.length)],
    understanding:
      questions.understanding[
        Math.floor(Math.random() * questions.understanding.length)
      ],
    emotional:
      questions.emotional[
        Math.floor(Math.random() * questions.emotional.length)
      ],
  };
};
...
async function createRealUserReading(
  spreadType,
  readingType,
  personalInfo,
  questions,
...
async function saveTestResults(
  ...
  console.log(
    `🎤 Sesli Okuma: ${voiceReading.id} (sesli - ${voiceReading.personalInfo.phone})`
  );
  console.log(
    `📝 Yazılı Okuma: ${writtenReading.id} (yazılı - ${writtenReading.personalInfo.email})`
  );
  console.log(
    `💳 Kredi: ${initialCredits} → ${finalCredits} (-${initialCredits - finalCredits})`
  );
  ...
  console.log(
    `💰 Toplam Maliyet: ${voiceReading.costCredits + writtenReading.costCredits} kredi`
  );
  console.log(
    `📊 Toplam Kart: ${voiceReading.cards.length + writtenReading.cards.length}`
  );
...
async function runRealUserTest(spreadType = 'love-spread') {
  ...
  try {
    ...
    const totalCreditsDeducted =
      voiceReading.costCredits + writtenReading.costCredits;
    console.log(
