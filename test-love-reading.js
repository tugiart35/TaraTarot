...
function generateRandomPersonalInfo() {
  ...
  const phoneNumber =
    phoneArea[Math.floor(Math.random() * phoneArea.length)] +
...
function generateRandomQuestions() {
  ...
  return {
    ...
    understanding:
      understandings[Math.floor(Math.random() * understandings.length)],
    emotional:
...
async function sendTestResults(voiceReading, writtenReading) {
  ...
  console.log('\n📋 Test Özeti:');
  console.log(
    `🎤 Sesli Okuma: ${results.voiceReading.id} (${results.voiceReading.communicationMethod})`
  );
  console.log(
    `📝 Yazılı Okuma: ${results.writtenReading.id} (${results.writtenReading.communicationMethod})`
  );
  console.log(
    `📊 Toplam Kart: ${results.voiceReading.cards.length + results.writtenReading.cards.length}`
  );
